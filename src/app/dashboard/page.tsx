...
export default function Dashboard() {
  const { user, session, profile, loading, canGeneratePrediction, decrementPredictions, acceptDisclaimer } = useAuth();
  ...
  useEffect(() => {
    // Auto-generate initial prediction only if user is authenticated, has accepted disclaimer, and can generate
    if (user && !loading && profile?.disclaimer_accepted && canGeneratePrediction()) {
      generatePrediction();
    }
  }, [user, loading, profile?.disclaimer_accepted]);
  ...
  if (user && !profile?.disclaimer_accepted) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center">
        <div className="text-center text-white">
          <h2 className="text-2xl font-bold mb-4">Welcome to PatternSight</h2>
          <p className="text-lg mb-6">Please review and accept our disclaimer to continue</p>
          <button
            onClick={() => setShowDisclaimerModal(true)}
            className="px-6 py-3 bg-gradient-to-r from-orange-500 to-pink-500 text-white rounded-lg font-semibold hover:from-orange-600 hover:to-pink-600 transition-all duration-200"
          >
            Review Disclaimer
          </button>
        </div>
        ...
        <DisclaimerModal
          ...
          onAccept={async (initials: string) => {
            await acceptDisclaimer(initials);
            setShowDisclaimerModal(false);
          }}
          onCancel={() => {
            setShowDisclaimerModal(false);
            // Redirect to home page if user cancels disclaimer
